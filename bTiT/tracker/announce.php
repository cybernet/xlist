...
if ($PRIVATE_ANNOUNCE) {
  ...
  if (!$respid || mysql_num_rows($respid) != 1)
     show_error("Invalid PID (private announce): $pid. Please redownload torrent from $BASEURL.");
      ...
      elseif ($rowpid["disabled"] != "no") {
         show_error("Sorry but the account you are attemptimg to use has been Disabled. Connection from peer ".$ip." on port ".$port." refused.");
      }
      //waittime
      elseif ($rowpid["WT"] > 0) {
        $wait = 0;
...
//Golden Torrents by CobraCRK
	  $q = mysql_query("SELECT free from namemap WHERE info_hash = '$info_hash'");
