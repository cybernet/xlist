...
block_begin("".CF_NO_GUEST_ACCESS."");
//    err_msg(ERROR.NOT_AUTHORIZED." ".MNU_TORRENT."!",SORRY."...");
	print ("<br/>");
	print ("<br/>");
	print ("<br/>");
	print ("".CF_LOGIN_2_ACCESS_TRACKER."");
	print ("<br/>");
	print ("<br/>");
	print ("<br/>");
block_end();
...
{
print("<tr><td align=\"right\" class=\"header\"> ".TORRENT.":</td><td class=\"lista\" align=\"center\"><a class \"index\" <a href=\"torrentget_cyberfun_torrent.php?id=".$row["info_hash"]."\">" . $row["filename"] . "</a><br><small><a href=report_this_.php?torrent=".$row["info_hash"]."><font color=Red>".CF_REPORT_THIS_TORRENT."</font></a> <a href=cyberfun_wish_list.php?do=add&torrent_id=".$row["info_hash"]."><font color=Green>".CF_ADD_2_WISH_LIST."</font></a></td></tr>\n");
} else {
print("<tr><td align=\"right\" class=\"header\"> ".TORRENT.":</td><td class=\"lista\" align=\"center\"><a class \"index\" <a href=\"get_cyberfun_torrent.php?id=".$row["info_hash"]."&f=" . rawurlencode($row["filename"]) . ".torrent\">" . $row["filename"] . "</a><br><small><a href=report_this_.php?torrent=".$row["info_hash"]."><font color=Red>".CF_REPORT_THIS_TORRENT."</font></a> <a href=cyberfun_wish_list.php?do=add&torrent_id=".$row["info_hash"]."><font color=Green>".CF_ADD_2_WISH_LIST."</font></a></td></tr>\n");
}
...
if (!empty($row["video"]))
{
if (preg_match("/.*v=(.*)/i",$row["video"],$v_id) || preg_match("/.*\/v\/(.*)/i",$row["video"],$v_id))
{
$src="<img border=\"0\" src=\"http://i1.ytimg.com/vi/$v_id[1]/default.jpg\">";
}
elseif (preg_match("/.*\/video\/([1-9]*)/i",$row["video"],$v_id))
{
$src="<img border=\"0\" src=\"http://dyn.ifilm.com/resize/image/stills/films/resize/istd/$v_id[1].jpg?width=130\">";
}
else
{
...
if (!empty($row["nfo"])){
print("<tr><td align=right class=\"header\"> NFO:</td><td align=left class=\"lista\" ><a href=view_cyberfun_nfo.php?info_hash=" . $row["info_hash"]. "><b>".CF_VIEW_NFO."</b></a></td></tr>\n");
}
...
if (file_exists($row["url"]))
  {
    ...
    <a name=\"#expand\" href=\"#expand\" onclick=\"javascript:ShowHide('files','msgfile');\">".CF_SHOW_HIDE."</td>
...
{
	print("<tr><td align=right class=\"header\">".CF_RESEED_REQUEST."</td><td class=\"lista\"  align=\"left\" ><a href=reseed.php?id=" . $row["info_hash"]. ">".image_or_link("images/reseedbutton.png","","Req Seed")."</a></td></tr>\n");
}
...
print("<tr><td align=right class=\"header\">".CF_STATISTIC."</td><td class=\"lista\" >".CF_NOT_AVAILABLE."</td></tr>");
}
else {
print("<tr><td align=right class=\"header\">".CF_STATISTIC."</td><td class=\"lista\" ><img src=stat.php?id=".$row["info_hash"]."></td></tr>");
}
if ($row["external"]=="yes")
   {
...
print("<tr><td align=right class=\"header\">".CF_THANKS."</td><td class=\"lista\" >");
$user = $CURUSER["uid"];
...
if ($srow["userid"]==0 && $uploaderrow["uploader"]!=$user)
{
 ...
 print("<input type=\"submit\" name=\"submit\" value=\"".CF_SAY_THANKS."\">");
...
if (!$subres || mysql_num_rows($subres)==0) {
   if($CURUSER["uid"]>1)
       $s = "<br /><br />\n<table width=\"95%\" class=\"lista\">\n<tr>\n<td align=\"center\">\n<a href=add_comment.php?id=" . $id . "&usern=".urlencode($CURUSER["username"]).">".NEW_COMMENT."</a>\n</td>\n</tr>\n";
   else
       $s = "<br /><br />\n<table width=\"95%\" class=\"lista\">\n";
     ...
     print("<br /><br />");
     if($CURUSER["uid"]>1)
       $s = "<br /><br />\n<table width=\"95%\" class=\"lista\"><tr><td colspan=\"3\" align=\"center\"><a href=add_comment.php?id=" . $id . "&usern=".urlencode($CURUSER["username"]).">".NEW_COMMENT."<br />" . $row["filename"]."</a></td></tr>\n";
     else
         $s = "<br /><br />\n<table width=95% class=lista>\n";
...
if($last > $online)
{
      $online = "".CF_USER_ONLINE."";
}
else
      $online = "".CF_USER_OFFLINE."";
// End Online Users
      if (is_valid_id($subrow['editedby']))
      {
        ...
        if (mysql_num_rows($res2) == 1)
        {
          ...
          //$s .= "<td colspan=2 valign=bottom width=100% height=15 class=lista border=\"0\" align=right>".$online."&nbsp;&nbsp;<font size=1 class=small>".LAST_EDITED_BY." <a href=userdetails.php?id=$subrow[editedby]><b>$userrow[username]</b></a> at " . $subrow["editedat"] . "</font>&nbsp;&nbsp;<a class=\"postlink\" href=\"#Top\">".image_or_link("./images/top.gif","","TOP")."</a></td></tr>\n";
		     $s .= "<tr align=right><td width=90% colspan=\"3\" valign=bottom class=lista align=right border=0>".$online."&nbsp;&nbsp;<font size=1 class=small>".LAST_EDITED_BY." <a href=userdetails.php?id=$subrow[editedby]><b>$userrow[username]</b></a>".CF_AT."".CF_SHIT."" . $subrow["editedat"] . "</font>&nbsp;&nbsp;<a class=\"postlink\" href=\"#Top\">".image_or_link("./images/top.gif","","TOP")."</a>".CF_SHIT."</td></tr>\n";
	   }
      }
