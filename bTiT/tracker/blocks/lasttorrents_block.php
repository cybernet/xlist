          ...
          if ( $CURUSER["uid"] == $data["uploader"] || $CURUSER["delete_torrents"] == "yes" )
           {
			...
			{
             echo "<a href=torrentget_cyberfun_torrent.php?id=".$data["hash"]."><img src='images/torrent.gif' border='0' alt='".DOWNLOAD_TORRENT."' title='".DOWNLOAD_TORRENT."' /></a>";
			 } else {
             echo "<a href=get_cyberfun_torrent.php?id=".$data["hash"]."&f=" . rawurlencode($data["filename"]) . ".torrent><img src='images/torrent.gif' border='0' alt='".DOWNLOAD_TORRENT."' title='".DOWNLOAD_TORRENT."' /></a>";
			}
...
if ($GLOBALS["enable_cutname"]==true)
{
       global $CUTNAME;
       if ($GLOBALS["usepopup"])
          echo "\t<td width=60% class=\"lista\"><a href=\"javascript:popdetails('details.php?id=" . $data['hash'] . "');\" title=\"" . VIEW_DETAILS . ": ".$data["filename"]."\">".((strlen($data["filename"])>='$CUTNAME')? substr($data["filename"],0,$CUTNAME)."...":$data["filename"])."</a>".($data["external"]=="no"?"":" (<span style=\"color:red\">EXT</span>)")."</td>";
       else
          echo "\t<td width=60% class=\"lista\"><a href=\"details.php?id=" . $data['hash'] . "\" title=\"" . VIEW_DETAILS . ": ".$data["filename"]."\">".((strlen($data["filename"])>='$CUTNAME')? substr($data["filename"],0,$CUTNAME)."...":$data["filename"])."</a>".($data["external"]=="no"?"":" (<span style=\"color:red\">EXT</span>)")."</td>";
}
