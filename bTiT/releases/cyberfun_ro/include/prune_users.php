...
elseif ($action=="view")
    ...
    $res=mysql_query("SELECT users.id, users.username, UNIX_TIMESTAMP(users.joined) as joined, UNIX_TIMESTAMP(users.lastconnect) as lastconnect, users_level.level from users INNER JOIN users_level ON users_level.id=users.id_level WHERE (users.id>1 AND users_level.id_level<3 AND UNIX_TIMESTAMP(joined)<(UNIX_TIMESTAMP()-$timeout)) OR (users.id>1 AND users.id_level != 13 AND users_level.id_level<7 AND UNIX_TIMESTAMP(lastconnect)<(UNIX_TIMESTAMP()-$timeout)) ORDER BY users_level.id_level DESC, lastconnect");
    ...
    elseif (mysql_num_rows($res)>0)
       ...
       $count=0;
       while ($rusers=mysql_fetch_array($res))
             {
             include("offset.php");
