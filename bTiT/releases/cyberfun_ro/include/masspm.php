...
$user_id_query = mysql_query("SELECT level FROM users_level WHERE id_level=$level")or sqlerr(__FILE__, __LINE__);
$user_rank=mysql_fetch_array($user_id_query);
$user_level=$user_rank['level'];

$user_id_query1 = mysql_query("SELECT level FROM users_level WHERE id_level=$level1")or sqlerr(__FILE__, __LINE__);
...
$i = 0;
//do database call
    $result_id = mysql_query("SELECT * FROM users where id > 1$where") or sqlerr(__FILE__, __LINE__);
...
// if matching ratio
  if($pick == 0)
  {
    ...
    if($ratio == $ratio1)
      ...
      if($pm)
      mysql_query("INSERT INTO messages (sender, receiver, added, subject, msg) VALUES ($sender,$user_id,UNIX_TIMESTAMP(),$subject,$msg)");
      }
  else continue;
  }
//if ratio X + greater
...
// PM sender if true
if($pm_sender)
mysql_query("INSERT INTO messages (sender, receiver, added, subject, msg) VALUES ($sender,".$CURUSER['uid'].",UNIX_TIMESTAMP(),$subject,$msg)");
//pm sent block
...
print("\n<form method=\"post\" name=\"masspm\" action=\"admincp.php?user=".$CURUSER["uid"]."&code=".$CURUSER["random"]."&do=masspm&action=post\">
 ...
 print("<tr><td class=header>To ".USER_LEVEL.":</td><td><select name=\"level1\">");
           ...
           $res1=mysql_query("SELECT id,level FROM users_level WHERE id_level>1 ORDER BY id_level");
