...
//if ($action=="signup")
{
   ...
   print("\n\t<td align=\"left\" class=\"lista\"><font color=\"red\">Hotmail is'nt allowed as a valid Email. Use your ISP email or Yahoo, Gmail ect ect. You most enter an valid email!</font></td>");
   print("</tr>");
}
?>
...
$random = rand($floor, $ceiling);
    ...
    $age = mysql_escape_string($_POST['age']);
	$gen = mysql_escape_string($_POST['gen']);
	@mysql_query("INSERT INTO users (username, password, random, email, style, language, flag, id_level, joined, pid, time_offset, invited_by, age, gender)
						VALUES (" .implode(",", array_map("sqlesc", array($utente, $wantpasshash, $random, $email, $idstyle, $idlangue, $idflag, $idlevel))) .", NOW(), '".md5(uniqid(rand(),true))."', '".$timezone."', $inviter, '$age', '$gen')");
