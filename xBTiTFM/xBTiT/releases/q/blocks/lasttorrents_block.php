...
?>
  ...
  if ($row)
  {
      ...
      echo "<a href=download.php?id=".$data["hash"]."&amp;f=" . rawurlencode($data["filename"]) . ".torrent onClick=\"javascript:pageTracker._trackPageview('/downloaded_from_last_torrents_block/".$data["hash"]."/" . urlencode($data["filename"]) . "');\"><img src='images/download.gif' border='0' alt='".$language["DOWNLOAD_TORRENT"]."' title='".$data["filename"]."' /></a>";
      echo "</td>";
