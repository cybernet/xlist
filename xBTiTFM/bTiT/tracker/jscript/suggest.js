...
function suggest(key,query,type) {
	if (key == 38) {
		...
		if (query.length > 3) {
			t = new Date();
			ajax.get('include/suggest.php?a='+type+'&q='+query+'&bla='+t.getTime(),update);
		} else {
			update('');
		}
	}
...
function suggest_users(key,query,type) {
	if (key == 38) {
		...
		if (query.length > 3) {
			t = new Date();
			ajax.get('include/suggest.php?a='+type+'&q='+query+'&bla='+t.getTime(),update_users);
		} else {
			update_users('');
		}
	}
...
function select(obj,mouse) {
	obj.style.backgroundColor = '#88a9da';
...
function unselect(obj,mouse) {
	obj.style.backgroundColor = '#E0ECF7';
...
function unselectAllOther(id) {
	for (i = 1; i <= count; i++) {
		if (i != id) {
			document.getElementById(i).style.backgroundColor = '#E0ECF7';
			document.getElementById(i).style.color = '#006699';
		}
	}
}