...
require_once "include/user_functions.php";
	...
	if(isset($_GET['act'])){
		...
		if($ACT==1){//--EDIT
			...
			$HTML.="
			<form action='{$TBDEV['baseurl']}/admin.php?action=themes&amp;act=4' method='post'><input type='hidden' value='{$TEM['id']}' name='ori' /><table width='50%'>
	...
	if(!isset($_GET['act'])){
		...
		<tr><td colspan='4'><a href='{$TBDEV['baseurl']}/admin.php?action=themes&amp;act=3'><span class='btn'>{$lang['themes_addnew']}</span></a></td></tr>
		<tr>
		...
		while($TE=mysql_fetch_array($TEMPLATES)){
			...
			<td><a href='{$TBDEV['baseurl']}/admin.php?action=themes&amp;act=1&amp;id=$TE[id]'>[{$lang['themes_edit']}]</a>
