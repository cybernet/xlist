...
if ( ! defined( 'IN_TBDEV_FORUM' ) )
{
    ...
    if ($newtopic)
    {
      ...
      $message1 = $CURUSER['username'] . " {$lang['forum_post_created_a_new_forum_topic']} [url={$TBDEV['baseurl']}/forums.php?action=viewtopic&topicid=$topicid&page=last]{$subject}[/url]";
    }
    else
    {
    ...
    $message = $CURUSER['username'] . " {$lang['forum_post_replied_to_the_thread']} [url={$TBDEV['baseurl']}/forums.php?action=viewtopic&topicid=$topicid&page=last]{$subject}[/url]";	
  if ($newtopic ){
